  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard label="Total Tickets" value={stats.total} colorClass="bg-pine-50 border-pine-200 text-pine-700" />
      </div>

      {/* Employee CTA */}
      {user.role === 'employee' && (
        <div className="bg-gradient-to-r from-pine-900 to-pine-700 rounded-xl p-6 text-white">
          <h2 className="text-lg font-semibold mb-1">Having an IT issue?</h2>
          <p className="text-pine-200 text-sm mb-4">
            Our AI assistant can solve most common issues instantly — no waiting required.
          </p>
          <Link
            to="/help"
            className="inline-block bg-white text-pine-900 font-semibold px-5 py-2 rounded-lg text-sm hover:bg-pine-50 transition-colors"
          >
            Get AI Help Now →
          </Link>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to={ticketsLink} className="text-sm text-pine-700 hover:underline">
            View all →
          </Link>
        </div>
