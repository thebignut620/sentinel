...
// ─── Mobile bottom nav icons (slightly larger w-5 h-5) ────────────────────────
const MOB = {
  dashboard: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>,
  tickets:   <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>,
  ...
  person:    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>,
...
function MobileBottomNav({ user, unreadNotifs }) {
  const { pathname } = useLocation();

  const links = user?.role === 'employee'
    ? [
        { to: '/dashboard',      icon: MOB.dashboard, label: 'Home' },
        { to: '/help',           icon: MOB.help,      label: 'AI Help' },
        { to: '/my-tickets',     icon: MOB.tickets,   label: 'Tickets' },
        { to: '/knowledge-base', icon: MOB.kb,        label: 'Knowledge' },
        { to: '/my-profile',     icon: MOB.person,    label: 'Profile' },
      ]
    : user?.role === 'it_staff'
    ? [
        { to: '/dashboard',       icon: MOB.dashboard, label: 'Home' },
        { to: '/tickets',         icon: MOB.tickets,   label: 'Tickets', badge: unreadNotifs },
        { to: '/knowledge-base',  icon: MOB.kb,        label: 'Knowledge' },
        { to: '/admin/analytics', icon: MOB.analytics, label: 'Analytics' },
        { to: '/my-profile',      icon: MOB.person,    label: 'Profile' },
      ]
    : [
        { to: '/dashboard',       icon: MOB.dashboard, label: 'Home' },
        { to: '/tickets',         icon: MOB.tickets,   label: 'Tickets', badge: unreadNotifs },
        { to: '/admin/analytics', icon: MOB.analytics, label: 'Analytics' },
        { to: '/admin/settings',  icon: MOB.settings,  label: 'Settings' },
        { to: '/my-profile',      icon: MOB.person,    label: 'Profile' },
      ];

  return (
    <nav
      className="md:hidden fixed bottom-0 left-0 right-0 bg-gray-900 border-t border-gray-800 z-50"
        ...
        {links.map(link => {
          ...
          return (
            <Link
              key={link.to}
              to={link.to}
              className="flex flex-col items-center justify-center gap-0.5 flex-1 relative min-h-[44px] active:bg-gray-800 transition-colors"
            >
              <span className={`transition-colors ${active ? 'text-pine-400' : 'text-gray-500'}`}>
                {link.icon}
              </span>
              {link.badge > 0 && (
                <span className="absolute top-1.5 right-1/4 translate-x-2 h-4 min-w-[16px] px-0.5 bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center">
                  {link.badge > 9 ? '9+' : link.badge}
                </span>
              )}
              <span className={`text-[9px] font-medium transition-colors ${active ? 'text-pine-400' : 'text-gray-600'}`}>
                {link.label}
              </span>
            </Link>
          );
...
export default function Layout() {
  ...
  const navItems = {
    ...
    admin: [
      ...
      { to: '/admin/analytics',        icon: ICONS.analytics,   label: 'Analytics' },
      { section: 'ATLAS' },
      { to: '/admin/settings',         icon: ICONS.settings,    label: 'ATLAS Settings' },
      { to: '/knowledge-base',         icon: ICONS.kb,          label: 'Knowledge Base' },
      { to: '/admin/company-profile',  icon: ICONS.building,    label: 'Industry Profile' },
  ...
  return (
    <div className="min-h-screen flex bg-gray-950">
      ...
      <aside
        className={`hidden md:flex flex-col shrink-0 bg-gray-900 border-r border-gray-800 transition-all duration-300 ease-in-out
          ${collapsed ? 'w-16' : 'w-56'}`}
      >
        {/* Header */}
      ...
      <main className="flex-1 overflow-auto flex flex-col min-w-0">
        ...
        {billingStatus?.isExpired && pathname !== '/admin/billing'
          ? <TrialExpiredScreen />
          : (
            <div className="flex-1 max-w-6xl mx-auto w-full px-3 py-4 sm:p-6 pb-24 md:pb-6 page-enter">
              <Outlet />
            </div>
          )
        }
